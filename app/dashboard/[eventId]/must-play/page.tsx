  ...
  const loadMustPlayList = async () => {
    try {
      ...
      if (eventSnap.exists()) {
        ...
        setSongs(data.mustPlayList || []);
      }
    } catch (error) {
      console.error('Error loading must play list:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddSong = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        await updateDoc(eventRef, {
          mustPlayList: arrayRemove(songData),
        });
        await updateDoc(eventRef, {
          ...
          {loading ? (
            ...
            <>
              <div className="bg-white border border-gray-200 rounded-lg p-6 shadow mb-6">
                <h2 className="text-xl font-bold mb-4">Add Song to Must Play List</h2>
              ...
              <div className="space-y-2">
                {songs.length === 0 ? (
                  <div className="text-center py-12 bg-gray-50 rounded-lg">
                    <p className="text-gray-600">No songs in your must play list yet.</p>
                  </div>
                ) : (
                  songs.map((song) => (
